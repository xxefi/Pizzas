...
public class OtpService : IOtpService
{
    ...
    {
        var storedOtp = await _redisCache.GetAsync<string>($"otp:{sessionId}");
        return storedOtp == otp;
    }
    public async Task SavePendingUserAsync(string sessionId, CreateUserDto userDto)
        => await _redisCache.SetAsync($"pendingUser:{sessionId}", userDto, TimeSpan.FromMinutes(5));

    public async Task<CreateUserDto?> GetPendingUserAsync(string sessionId)
        => await _redisCache.GetAsync<CreateUserDto>($"pendingUser:{sessionId}");
